...
//      String filePath = (javax.servlet.http.HttpServletRequest)request.getSession().getServletContext().getRealPath("/") + "resources/images/logoImages/llhlogo.png";
        ...
        String filePath = path + "logo.jpg";  //TODO
        try
        {  

            BufferedImage bim = getQR_CODEBufferedImage(qrUrl, BarcodeFormat.QR_CODE, 400, 400, getDecodeHintType());
            return addLogo_QRCode(bim, new File(filePath), new LogoConfig(), productName);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    *          日期                 修改人                 修改说明
    ********************************************************/
    public static String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName)
    {
        try
        {
            /**
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
            ...
            ImageIO.write(image, "jpg", new File(path+"TDC-" + new Date().getTime() + "testLogo.png")); //TODO
